...
- Optimising without baseline measurements first
- Applying micro-optimisations before profiling (premature optimisation)
- "It should be faster" without benchmarks to prove it
- Optimising code that isn't on the hot path
- Adding caching without understanding invalidation strategy
- Making code harder to read for marginal speed gains
- Not re-measuring after applying a fix
- Optimising based on assumptions rather than profiler output
- Ignoring database queries while tuning application code
